function [apriori2,apriorierror2] = apriorimodel_bafim_flipchem_plasmaline(apriori,apriorierror,heights,fit_altitude)
%
%
...
% d_time    timestap from the raw data
    ...
    persistent r_apriori_smoother r_apriorierror_smoother
    ...
    %
    % the value can be set using the global variable bafim_flipchem_model_error. Negative values mean that
    % the Flipchem correction is not done. For example, include the following in  
    % GUISDAP startup script or in the GUISDAP for dummies GUI "special" box to skip the Flipchem correction
    % 
    % global bafim_flipchem_model_error
    % bafim_flipchem_model_error=-1
    %
    %
    if isempty(bafim_flipchem_model_error)
        flipchem_modErr_std = .1;
    else
        flipchem_modErr_std = bafim_flipchem_model_error;
    end
    disp(['Setting flipchem_modErr_std = ', num2str(flipchem_modErr_std)])
    ...
    if istep==2
        ...
        copyfile(which('apriorimodel_bafim_flipchem_plasmaline.m'),fullfile(result_path,'apriorimodel_bafim_flipchem_plasmaline.m'));
        copyfile(which('apriorimodel_plasmaline.m'),fullfile(result_path,'apriorimodel_plasmaline.m'));
    end
    ...
    % counters to re-initialize the filter after data gaps longer than 3000 s
    if tstep > 3000
        tfromstart = 0;
    else
    ...
    if tfromstart > 120 & istep>1
        ...
        for hind = 1:nhei
            ...
            SSlim = 25;
            % this does not work with the plasma line prior
        ...
        NOp = r_param(:,1).*NaN;
        NO  = r_param(:,1).*NaN;
        % flipchem_modErr_std>0 indicates that flipchem should be used
        if flipchem_modErr_std>0
        ...
        while ~savesuccess
            savesuccess = true;
            try
                save(outfile,'BAFIM_G','r_param','r_error','r_status','r_param_filter','r_error_filter','r_param_rcorr','r_error_rcorr','r_apriori_smoother','r_apriorierror_smoother','r_dp','O2p','NOp','NO','kdeb','-append');
            catch
                savesuccess = false;
                itry = itry + 1;
                pause(5);
    ...
    r_apriori_smoother = apriori2;
    r_apriorierror_smoother = apriorierror2;
    
    % prior model with the plasma line data
    [aprioripl,apriorierrorpl] = apriorimodel_plasmaline(apriori,apriorierror,heights,fit_altitude);
    
    % add Ne information from the plasma line prior
    apriori2(:,1) = apriori2(:,1)./apriorierror2(:,1).^2 + aprioripl(:,1)./apriorierrorpl(:,1).^2;
    apriorierror2(:,1) = sqrt(1./(1./apriorierror2(:,1).^2 + 1./apriorierrorpl(:,1).^2));
    apriori2(:,1) = apriori2(:,1).*apriorierror2(:,1).^2;
    ...
    disp('apriorimodel_bafim_flipchem_plasmaline')
